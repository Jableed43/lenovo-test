...
class CarManager {
    array = []
    /* getItemsByKey returns an array where key parameter is the category your cars are sorted by, can be a color as 'red', or a size as 'medium' */
    getItemsByKey(key) {
...
/* addNewCategoryList creates a new category inside array based on specific key. name parameter is the category you want your items to be separated and sorted by. Can be a new color, a new size or any other category. */
...
try {
    ...
    console.log(`The key ${carKey} does not exist on list you can create it by using addNewCategoryList function`);
}
}
